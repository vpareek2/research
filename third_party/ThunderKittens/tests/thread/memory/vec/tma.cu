...
struct test_load { // load with TMA, write out normally
    using dtype = T;
    template<int S, int NW> using valid = std::bool_constant<NW == 1 && (S*16 <= 256 || (S*16*sizeof(dtype)) % 128 == 0) && ( ( sizeof(T) != 1 ) || S%2 == 0 )>; // WARNING: can have misalignment if multiple shared vectors are initialized in an array
    static inline const std::string test_identifier = std::is_same_v<T, kittens::bf16> ? "tma_load_vec_gmem=bf16" :
                                                      ...
                                                      std::is_same_v<T, kittens::int8> ? "tma_load_vec_gmem=int8" :
                                                      std::is_same_v<T, kittens::uint8> ? "tma_load_vec_gmem=uint8" :
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int c = 0; c < input.rows(); c++) for(int d = 0; d < input.cols()/shared_vec.length; d++) {
                if(threadIdx.x == 0) kittens::tma::expect(smem_semaphore, shared_vec);
                if(threadIdx.x == 0) kittens::tma::load_async(shared_vec, input, {a, b, c, d}, smem_semaphore);
                kittens::wait(smem_semaphore, tic);
                kittens::warp::store(output, shared_vec, {a, b, c, d});
                tic^=1;
            }
        }
    }
...
struct test_store { // load normally, store with TMA
    ...
    static inline const std::string test_identifier = std::is_same_v<T, kittens::bf16> ? "tma_store_vec_gmem=bf16" :
                                                      ...
                                                      std::is_same_v<T, kittens::int8> ? "tma_store_vec_gmem=int8" :
                                                      std::is_same_v<T, kittens::uint8> ? "tma_store_vec_gmem=uint8" :
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int c = 0; c < input.rows(); c++) for(int d = 0; d < input.cols()/shared_vec.length; d++) {
                kittens::warp::load(shared_vec, input, {a, b, c, d});
                __syncwarp();
                if(threadIdx.x == 0) kittens::tma::store_async(output, shared_vec, {a, b, c, d});
                if(threadIdx.x == 0) kittens::tma::store_async_read_wait();
            }
        }
    }
};
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int c = 0; c < input.rows(); c++) for(int d = 0; d < input.cols()/shared_vec.length; d++) {
                kittens::warp::load(shared_vec, input, {a, b, c, d});
                __syncwarp();
                if(threadIdx.x == 0) kittens::tma::store_add_async(output, shared_vec, {a, b, c, d});
                if(threadIdx.x == 0) kittens::tma::store_add_async(output, shared_vec, {a, b, c, d});
                if(threadIdx.x == 0) kittens::tma::store_async_read_wait();
            }
        }
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int c = 0; c < input.rows(); c++) for(int d = 0; d < input.cols()/shared_vec.length; d++) {
                kittens::warp::load(shared_vec, input, {a, b, c, d});
                __syncwarp();
                if(threadIdx.x == 0) kittens::tma::store_min_async(output, shared_vec, {a, b, c, d});
                if(threadIdx.x == 0) kittens::tma::store_async_read_wait();
            }
        }
    }
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int c = 0; c < input.rows(); c++) for(int d = 0; d < input.cols()/shared_vec.length; d++) {
                kittens::warp::load(shared_vec, input, {a, b, c, d});
                __syncwarp();
                if(threadIdx.x == 0) kittens::tma::store_max_async(output, shared_vec, {a, b, c, d});
                if(threadIdx.x == 0) kittens::tma::store_async_read_wait();
            }
        }
    }
...
struct tma_sweep_gmem_type_1d {
    static void run(test_data &results) {
        ...
        tma_sweep_size_1d<test<kittens::int8>, MAX_S, NUM_WORKERS, args...>::run(results);
        tma_sweep_size_1d<test<kittens::uint8>, MAX_S, NUM_WORKERS, args...>::run(results);
    }
};
...
void thread::memory::vec::tma::tests(test_data &results) {
    std::cout << " ----- Starting ops/thread/memory/vec/tma tests! -----\n" << std::endl;
