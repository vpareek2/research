...
| `configs/jaxtitan/cloud_4gpu_dense_tp_muon_validation.toml` | dense tensor parallelism with exact logical-matrix Muon |
...
| `configs/jaxtitan/cloud_4gpu_dense_fsdp_tp_muon_validation.toml` | dense FSDP+TP with exact logical-matrix Muon |
| `configs/jaxtitan/cloud_4gpu_dense_zero2_tp_adamw_validation.toml` | dense ZeRO-2+TP composition |
| `configs/jaxtitan/cloud_4gpu_dense_zero2_tp_muon_validation.toml` | dense ZeRO-2+TP with exact logical-matrix Muon |
...
rank-2 hidden matrices through the exact logical-matrix `dist_muon_exact`
reference backend. The route passed the four-H100 17-step matrix and composed
64-step stress matrix recorded in the distributed-Muon runbook. These configs
validate correctness and artifact shape, not representative throughput.
