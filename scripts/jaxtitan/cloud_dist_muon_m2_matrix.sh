...
usage() {
  cat <<'EOF'
Usage: scripts/jaxtitan/cloud_dist_muon_m2_matrix.sh [--overwrite]

Run four matched duplicated/distributed 64-step Muon profile pairs on one
four-GPU node. The script verifies hardware and data, dumps HLO, runs
inspect/eval/sample, and creates a lightweight evidence archive. Full profiler
trees remain under runs/ for optional selective transfer.
EOF
}
...
capture_name="dist_muon_m2_${timestamp}"
...
capture_run_evidence() {
  ...
  mkdir -p "$destination"
  for path in config.resolved.toml diagnostics events.jsonl metrics summaries; do
...
package_results() {
  ...
  if ! uv run python scripts/jaxtitan/analyze_dist_muon_m2_results.py \
  ...
  du -sh "$archive"
  marker "ALL_DIST_MUON_M2_RUNS_COMPLETE ${archive}"
