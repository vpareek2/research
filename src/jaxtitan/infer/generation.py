...
def make_prefill_step(graph: Any) -> Callable[[InferenceState, PrefillBatch, KVCache], PrefillOutput]:
    ...
    def _compiled(
        ...
        return prefill_model(graph, model_state, input_ids, positions, attention_mask, cache)
...
def make_decode_step(graph: Any) -> Callable[[InferenceState, DecodeBatch, KVCache], DecodeOutput]:
    ...
    def _compiled(
        ...
        return decode_model(graph, model_state, token_ids, positions, attention_mask, cache)
...
def prefill(graph: Any, state: InferenceState, batch: PrefillBatch, cache: KVCache) -> PrefillOutput:
    """Run compiled prefill for a graph/state/cache."""

    return make_prefill_step(graph)(state, batch, cache)


def decode_one(graph: Any, state: InferenceState, batch: DecodeBatch, cache: KVCache) -> DecodeOutput:
    """Run compiled one-token decode for a graph/state/cache."""

    return make_decode_step(graph)(state, batch, cache)
...
def generate_tokens(
    ...
    cache = init_kv_cache(model_spec, batch_size=batch_size, max_cache_len=total_len)
    positions = jnp.broadcast_to(jnp.arange(prompt_len, dtype=jnp.int32)[None, :], prompt_ids.shape)
    prefill_batch = PrefillBatch(
        input_ids=prompt_ids,
        positions=positions,
        attention_mask=jnp.ones_like(prompt_ids, dtype=jnp.bool_),
    )
    prefill_step = make_prefill_step(graph)
    decode_step = make_decode_step(graph)
    prefill_out = prefill_step(state, prefill_batch, cache)
    next_logits = prefill_out.next_token_logits
    ...
    for offset in range(spec.max_new_tokens):
        ...
        attention_mask = jnp.arange(total_len, dtype=jnp.int32)[None, :] <= decode_positions[:, None]
